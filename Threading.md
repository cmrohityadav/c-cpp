...
```
### timed_mutex
- std::timed_mutex is a mutex that allows a thread to wait for a lock for a specified maximum amount of time
- Thread lock acquire karne ke liye maximum kitne time tak wait karega, ye hum specify kar sakte hain. 
- Agar given time ke andar lock nahi mila, to function false return karega aur thread aage continue kar sakta hai.
```cpp
#include<iostream>
#include<thread>
#include<mutex>
...
- Same thread mutex ko multiple times lock kar sakta hai, aur agar mutex kisi doosre thread ke paas locked hai, to specified time tak wait karega. Agar given time mein lock nahi mila, to false return karega
```cpp
#include <iostream>
#include <thread>
#include <mutex>
...
### shared_mutex
- Multiple threads ek saath shared lock le sakte hain
- Sirf Reading k liye,Read krte waqt multiple threads shared lock le skte hai
- Par write krte waqt ek hi thread lock lega aur sab wait karege(reader and writer dono wait krege)
```cpp
#include <iostream>
#include <thread>
#include <mutex>

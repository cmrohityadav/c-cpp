...
```
### C- Style String Methods
- C-style string = characters ka array + last me '\0'
```cpp
#include<iostream>
#include<cstring>
using namespace std;
...
```
- C++ me struct and class almost same hi hai
---
 | Point                          | `struct`                      | `class`                                  |
| ------------------------------ | ----------------------------- | ---------------------------------------- |
| **1. Default Access Modifier** | `public`                      | `private`                                |
| **2. Default Inheritance**     | `public` inheritance          | `private` inheritance                    |
...
- Har copy same object ko own karti hai
- Internally ek reference count maintain hota hai
- Jab ek `shared_ptr` copy hota hai → reference count +1
- Jab ek `shared_ptr` destroy/reset hota hai → reference count -1
- Jab reference count 0 ho jata hai → object automatically destroy ho jata hai
- Isliye manually delete karne ki zarurat nahi hoti
```txt
std::make_shared<int>(100);

shared_ptr banane ka recommended function hai

Ye dynamically allocated int object banata hai,
Usko 100 se initialize karta hai,
Aur directly std::shared_ptr<int> return karta hai

std::shared_ptr<int> p = std::make_shared<int>(100); ~ std::shared_ptr<int> p(new int(100));

```
- Object tab destroy hota hai jab last shared_ptr owner bhi destroy ho jata hai
```cpp
#include <iostream>
#include <memory>
